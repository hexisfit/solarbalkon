export default async function handler(req, res) {
  ...
  // System SKUs (rows 2-5)
  const SYSTEM_SKUS = ['ZDSF2400AC+', 'STREAM AC Pro', 'AE-FS2.0-2H2', 'F3800-PLUS'];
  const SYSTEM_NAMES = {
    'ZDSF2400AC+': 'Zendure SolarFlow 2400 AC+',
    'STREAM AC Pro': 'EcoFlow STREAM AC Pro',
    'AE-FS2.0-2H2': 'Deye AE-FS2.0-2H2',
    'F3800-PLUS': 'Anker SOLIX F3800',
  };
  // Column D=Zendure, E=EcoFlow, F=Deye, G=Anker
  const SYSTEM_COLS = { 3: 'zendure', 4: 'ecoflow', 5: 'deye', 6: 'anker' };
  const PRICE_COL = 7;
  const OPTIONAL_COL = 8;
  ...
  try {
    ...
    // Parse systems (scan first rows for system SKUs)
    const prices = {};
    for (let i = 1; i < Math.min(rows.length, 10); i++) {
      const sku = rows[i][1];
      const name = SYSTEM_NAMES[sku];
      const rawPrice = rows[i][PRICE_COL];
      if (name && rawPrice) {
      ...
      // Stop scanning systems when we hit a non-system row
      if (sku && !SYSTEM_SKUS.includes(sku)) break;
    }

    // Find where components start (after systems)
    let componentStart = 1;
    for (let i = 1; i < Math.min(rows.length, 10); i++) {
      if (SYSTEM_SKUS.includes(rows[i][1])) {
        componentStart = i + 1;
      } else if (rows[i][0] && rows[i][1]) {
        componentStart = i;
        break;
      }
    }
    ...
    const components = [];
    for (let i = componentStart; i < rows.length; i++) {
      const name = rows[i][0];
      const sku = rows[i][1];
      const qty = parseInt(rows[i][2]) || 1;
      const rawPrice = rows[i][PRICE_COL];
      const note = (rows[i][OPTIONAL_COL] || '').toLowerCase();

      if (!name || !rawPrice) continue;

      // Skip system SKUs completely — they're not accessories
      if (SYSTEM_SKUS.includes(sku)) continue;

      const eur = parseFloat(rawPrice.replace(',', '.'));
      if (isNaN(eur)) continue;

      // Determine compatible systems from system columns
      const systems = [];
