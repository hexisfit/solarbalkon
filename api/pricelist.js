export default async function handler(req, res) {
  ...
  const brand = (name, sku) => {
    ...
    // Dyness
    if (['BX51100', 'DL5.0C', 'POWERBRICK', 'S51100', 'SBDU', 'HV4F',
         'BDU_HV4F', 'HUB_HV4F', 'BR_11SHV4F'].some(p => s.includes(p)) ||
        n.includes('dyness')) return 'Dyness';

    // FlashFish
    if (s.startsWith('FF') || s.includes('F132') || n.includes('flashfish')) return 'FlashFish';

    // AGENT
    if (['HSI_3500', 'HSI_5500', 'SP-ESS', 'J3000L-24', 'J5500HP',
         '51_2V100', '24V200AH'].some(p => s.includes(p)) ||
        n.includes('agent')) return 'AGENT';

    // Deye
    if (['SUN-', 'BOS-G', 'BOS-A', 'GB-S', 'GB-L', 'MGB-L',
         'SE-G', 'SE-F', 'RW-M', 'RW-F', 'AI-W5.1', 'AE-F2.0', 'GE-F',
         'ASM20', 'ASM02', 'HVB750', '3U-HRACK', '3U-LRACK',
         'BOS-GM5.1', 'BOS-G-PDU', 'BOS-A-PDU', 'BOS-A-RACK',
         'GB-SL', 'GB-L-HVCBWS', 'GB-L-BB', 'DEYE', 'BK01'].some(p => s.includes(p)) ||
        n.includes('deye')) return 'Deye';
